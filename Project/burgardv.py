...
pygame.init()
pygame.display.set_caption('Project Test 5')
...
viewButtons = [
    Button((205, 205, 205), 15, 440, 160, 40, 'Default'),
            ...
            if g[i][j].obstacle:
                if profile[2] is not None:
                    if profile[0][0] is None:
                        pygame.draw.rect(gridEnvironment, profile[2], block)
                        continue
                    if profile[0][0]:
                        if profile[1]:
            ...
            if g[i][j].inrange:
                if profile[3] is not None:
                    if profile[0][1] is None:
                        pygame.draw.rect(gridEnvironment, profile[3], block)
                        continue
                    if profile[0][1]:
                        if profile[1]:
            ...
            if g[i][j].explored:
                if profile[4] is not None:
                    if profile[0][2] is None:
                        pygame.draw.rect(gridEnvironment, profile[4], block)
                        continue
                    if profile[0][2]:
                        if profile[1]:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for button in viewButtons:
                if button.isOver(mousepanel):
                    index = viewButtons.index(button)
